    ...
    stages {
        stage('Create web directory')
        {
            ...
            steps{
                ...
                sh 'rm -rf "$WORKSPACE/deploy-web"'
                //Create the directory
                sh 'mkdir -p "$WORKSPACE/deploy-web"'
        ...
        stage('Create the Apache httpd container') {
            ...
            sh 'docker run -dit --name apache1 -p 9000:80 -v "$WORKSPACE/deploy-web:/usr/local/apache2/htdocs/" httpd'
            }
        }
        stage('Copy the web application to the container directory') {
            steps {
                echo 'Copying web application...'             
                sh 'cp -r web/* "$WORKSPACE/deploy-web"'
            }
        }
